...
module Resque::Durable
  class DurableTest < Minitest::Test
    describe 'Durable queue' do
      { enqueue: -> { MailQueueJob.enqueue(:foo, :bar) }, enqueue_at: -> { MailQueueJob.enqueue_at(5.seconds.from_now, :foo, :bar) } }.each do |method, subject|
        describe "##{method}" do
          before do
            Timecop.freeze
            QueueAudit.delete_all
            GUID.stubs(:generate).returns('abc/1/12345')
            Resque.stubs(:enqueue).with(Resque::Durable::MailQueueJob, :foo, :bar, 'abc/1/12345')
            Resque.stubs(:enqueue_at).with(5.seconds.from_now, Resque::Durable::MailQueueJob, :foo, :bar, 'abc/1/12345')
